...
The updater is a native-only adapter around Tauri's minisign-verifying updater. It is absent unless a non-empty public key is embedded at compile time, so development builds make no update request. Checks are explicit and pinned to the project's HTTPS GitHub release manifest; the webview receives only configured/current-version/available-version state. Installation rechecks the requested version to prevent stale selection, rejects concurrent work and pending database restore, verifies the downloaded artifact natively, replaces only the application bundle, and requests restart. No updater capability is granted directly to the webview.
