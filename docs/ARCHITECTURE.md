...
The updater is a native-only adapter around Tauri's minisign-verifying updater. Its public trust anchor is committed with the application; the corresponding encrypted private key remains outside the repository. Checks require an explicit user action and are pinned to the project's HTTPS GitHub release manifest; merely starting the application makes no update request. The webview receives only configured/current-version/available-version state. Installation rechecks the requested version to prevent stale selection, rejects concurrent work and pending database restore, verifies the downloaded artifact natively, replaces only the application bundle, and requests restart. No updater capability is granted directly to the webview.

## macOS build and distribution

Vite builds immutable React assets that Tauri embeds in `PersonalAssistant.app`. Cargo and npm lock files pin transitive dependencies. CI builds on Apple Silicon, runs Rust/UI/integration/migration tests and audits, generates an SBOM, applies hardened-runtime entitlements, signs nested code and the app with Developer ID Application, verifies it, notarizes, staples, then builds/verifies `PersonalAssistant-x.x.x.dmg` containing the app and Applications shortcut. Credentials live only in CI secret storage. The same Developer-ID-signed app is archived as the updater payload and signed independently with the offline updater key; a deterministic static manifest binds version, platform, signature, and the exact GitHub release asset URL. Application Support and Keychain paths are outside the replaced bundle.
