...
export function App({ api = tauriSettingsApi }: { api?: SettingsApi }) {
  ...
  async function chooseRestoreBackup() {
    ...
    try {
      const source = await open({ multiple: false, directory: false, filters: [{ name: "Personal Assistant encrypted backup", extensions: ["pabackup"] }] });
      if (typeof source === "string") setPendingRestoreSource(source);
    } catch (reason) {
      setBackupMessage(typeof reason === "string" ? reason : "The encrypted backup could not be selected safely.");
    }
  }

  async function confirmRestoreBackup() {
    if (!pendingRestoreSource || backupPassword.length < 12) return;
    setBackupAction(true); setBackupMessage("");
    try {
      const message = await backupApi.prepareRestore(pendingRestoreSource, backupPassword);
      setPendingRestoreSource(null); setBackupPassword(""); setBackupPasswordConfirmation("");
      setBackupMessage(message);
      await backupApi.restartForRestore();
    } catch (reason) {
